...
class ModulesLightTest : BasePlatformTestCase() {
    fun testTypingSpeedInitialization() {
        ...
        PrefState.Companion.enableModule(typingSpeed.getPreferenceId())
    ...
    fun testBehavioralTelemetryAggregatorInitialization() {
        ...
        PrefState.Companion.enableModule(aggregator.getPreferenceId())

        configService.getAvailableModules()
            .find { it.id == "BehavioralTelemetryAggregator" }
            ?.submodules
            ?.forEach { PrefState.Companion.enableModule(it.id) }
