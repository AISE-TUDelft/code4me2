...
// Create a dummy inline completion event (you can use a fake one)
    ...
    fun testCollectDataWithDisabledModule() {
        ...
        PrefState.disableModule(typingSpeedPreferenceId)
        ...
        val hasTypingSpeedData = result.any { record -> 
            record.type == Record.Type.BEHAVIORAL_TELEMETRY && record.containsKey(typingSpeedKey)
        }

        assertFalse(hasTypingSpeedData, "collectData should not return typing speed data when the TypingSpeed module is disabled")

        // Re-enable the module for other tests
        PrefState.enableModule(typingSpeedPreferenceId)

        val secondResult = getModuleManager(project).collectData(request)

        assertNotNull(secondResult, "collectData should not return null after re-enabling the module")

        val hasTypingSpeedDataAfterEnable = secondResult.any { record ->
            record.type == Record.Type.BEHAVIORAL_TELEMETRY && record.containsKey(typingSpeedKey)
        }

        assertTrue(hasTypingSpeedDataAfterEnable, "collectData should return typing speed data after re-enabling the TypingSpeed module")
    }
    ...
    fun testFileContextRetrievalModule() {
        ...
        PrefState.disableModule(fileContextModuleId)
        ...
        val suffixKey = Record.key<String>("suffix")
        val fileNameKey = Record.key<String>("file_name")

        // Check that no record contains any FileContextRetrievalModule data
        val hasFileContextData = result.any { record -> 
        ...
        assertFalse(hasFileContextData, "collectData should not return any file context data when the FileContextRetrievalModule is disabled")

        // Now re-enable the module
        PrefState.enableModule(fileContextModuleId)

        // But disable specific data points: prefix and suffix
        PrefState.setPreferenceValue(fileContextModuleId, "context.include.prefix", "false")
        PrefState.setPreferenceValue(fileContextModuleId, "context.include.suffix", "false")
        ...
        assertNotNull(contextRecord, "collectData should return a CONTEXT record when FileContextRetrievalModule is enabled")

        // Verify that prefix and suffix are not included
        assertFalse(contextRecord!!.containsKey(prefixKey), "collectData should not return prefix data when it's disabled")
        assertFalse(contextRecord.containsKey(suffixKey), "collectData should not return suffix data when it's disabled")

        // But file_contents and file_name should be included
        assertTrue(contextRecord.containsKey(fileContentsKey), "collectData should return file_contents data when it's enabled")
        assertTrue(contextRecord.containsKey(fileNameKey), "collectData should return file_name data when it's enabled")

        // Reset preferences for other tests
        PrefState.setPreferenceValue(fileContextModuleId, "context.include.prefix", "true")
        PrefState.setPreferenceValue(fileContextModuleId, "context.include.suffix", "true")
    }
}
