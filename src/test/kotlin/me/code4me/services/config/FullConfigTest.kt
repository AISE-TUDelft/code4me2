...
class FullConfigTest : HeavyPlatformTestCase() {
    ...
    fun testModuleInitializationFromConfig() {
        ...
        assertTrue("BehavioralTelemetryAggregator should be initialized", 
            initializedModuleIds.contains("BaseBehavioralTelemetryAggregator"))
        assertTrue("ContextualTelemetryAggregator should be initialized", 
            initializedModuleIds.contains("BaseContextualTelemetryAggregator"))
        assertTrue("contextAggregator should be initialized", 
            initializedModuleIds.contains("BaseContextAggregator"))

        // Verify submodules are also initialized
        assertTrue("TimeSinceLastAcceptedCompletion should be initialized", 
            initializedModuleIds.contains("TimeSinceLastAcceptedCompletion"))
        assertTrue("EditorContextRetrievalModule should be initialized", 
            initializedModuleIds.contains("EditorContextRetrievalModule"))
        assertTrue("FileContextRetrievalModule should be initialized", 
            initializedModuleIds.contains("FileContextRetrievalModule"))
        assertTrue("MultiFileContextRetrievalModule should be initialized",
            initializedModuleIds.contains("MultiFileContextRetrievalModule"))
        assertTrue("TimeSinceLastShownCompletion should be initialized",
            initializedModuleIds.contains("TimeSinceLastShownCompletion"))
        assertTrue("TypingSpeed should be initialized",
            initializedModuleIds.contains("TimeSinceLastAcceptedCompletion"))
